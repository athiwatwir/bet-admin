...
class CasinoGameComponent 
{
    public function hello() {
        ...
        //     $this->getGateway($game_data->api_url).'
        //     ?cmd=Hello&vendorId='.$this->getVendorId($game_data->api_token).'
        //     &signature='.$this->getSignature($game_data->api_token)
        // );
    ...
    public function createPlayer($user_id) { // MemberRegister
        ...
        $response = Http::asForm()->post(
            $this->getGateway($game_data->api_url).'
            ?cmd=MemberRegister
            &vendorId='.$this->getVendorId($game_data->api_token).'
            &signature='.$this->getSignature($game_data->api_token).'
            &user='.$user->username.'
            &password='.$user->phone.'
    ...
    public function getBalance() { // GetBalance
        $game_data = $this->getGameData();
        $user = $this->getUserData($user_id);
    ...
    public function transferOut() { // ChangeBalance Withdraw
        $game_data = $this->getGameData();
        $user = $this->getUserData($user_id);
    ...
    private function getGateway($api_url) {
        foreach($api_url as $url) {
            if($url['name'] == 'Gateway') return $url['url'];
        }
    }
